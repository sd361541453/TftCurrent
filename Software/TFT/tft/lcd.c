...
void LCD_WR_REG(u16 regval) {
	SPILCD_CS_LOW; //LCD_CS=0
	SPILCD_RS_LOW;
	SPI_WriteByte(regval & 0x00FF);
	SPILCD_CS_HIGH; //LCD_CS=1
}
...
void LCD_WR_DATA_NOCSRS(u16 data) {
	SPI_WriteByte(data >> 8);
	SPI_WriteByte(data);
}
...
//初始化lcd
void LCD_Init(void) {
	SPILCD_RST_HIGH; //LCD_RST=1
	delay_ms(50); // delay 20 ms

	SPILCD_RST_LOW; //LCD_RST=0	 //SPI接口复位
...
void showhanzi32(unsigned int x, unsigned int y, unsigned char index) {
	...
	for (j = 0; j < 32; j++) {
		...
		for (k = 0; k < 4; k++) {
			for (i = 0; i < 8; i++) {
				if ((*temp & (1 << i)) != 0) {
					LCD_WR_DATA_NOCSRS(POINT_COLOR);
				} else {
					LCD_WR_DATA_NOCSRS(BACK_COLOR);
				}
			}
			temp++;
		}
